...
// Hand-drawn delta glyph — reads like a sketchbook triangle being traced
// stroke by stroke: first a single diagonal stroke, then two strokes
// meeting at a peak, then the closed triangle with its inner triangle.
// Rendered as plain translucent white lines (no fill, no hatching) so the
// copy behind it stays legible — the lines sit *over* the words rather than
// blocking them. Each section's instance draws itself in slowly (~13s
// ease-out) once it scrolls into view, so the motion is easy to catch
